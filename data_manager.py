...
class Market1501(object):
    ...
    def __init__(self, root='data', **kwargs):   #**kwargs一般用作一些不确定参数
        self.dataset_dir = osp.join(root, self.dataset_dir)    #os.path.join(path1[, path2[, ...]])	把目录和文件名合成一个路径 这儿表示数据集的绝对路径
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train') #训练集路径
...
#     data=Market1501(root='data所在路径')
        #需要获得文件下的信息 文件路径、图片信息（REID、CAMID）、图片数量
    ...
    def _check_before_run(self):                      #判断文件即数据集是否存在 不存在则相应的进行提示
    ...
    def _process_dir(self, dir_path, relabel=False):    #
        img_paths = glob.glob(osp.join(dir_path, '*.jpg')) #拿出dir_path路径下所有.jpg文件 glob函数的作用
        pattern = re.compile(r'([-\d]+)_c(\d)')      #正则 取出前面的reid和cameraid

        pid_container = set()               #如果直接训练，数据由于有测试集，前面的reid有空档，所以对训练集进行重新排
        for img_path in img_paths:
            pid, _ = map(int, pattern.search(img_path).groups())  #map() 会根据提供的函数对指定序列做映射 ，其会返回两个参数 一个是reid、另一个是camid
            if pid == -1: continue  # junk images are just ignored    #数据集本身有些垃圾数据，对其进行淘汰
            pid_container.add(pid)
        pid2label = {pid:label for label, pid in enumerate(pid_container)}   #原先的数据集的reid与现在重新排序的reid进行组合成键值对  如：1500:750

        dataset = []
        for img_path in img_paths:
            pid, camid = map(int, pattern.search(img_path).groups())   #search().group() 返回匹配的所有结果 具体见博客
            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 1501  # pid == 0 means background
            assert 1 <= camid <= 6
            camid -= 1 # index starts from 0       照相机从0开始
            if relabel: pid = pid2label[pid]       #pid进行重新排
            dataset.append((img_path, pid, camid))

        num_pids = len(pid_container)   #返回reid个数    这儿图片个数与reid个数是不同的，reid表示有多少个人，而图片是所有的集合
        num_imgs = len(dataset)         #返回图片个数
        return dataset, num_pids, num_imgs

class CUHK03(object):           #另一数据集，具体操作跟Market1501数据集相似
    """
    CUHK03
    Reference:
...
class DukeMTMCreID(object):    #DUke数据集
    """
    DukeMTMC-reID
...
class iLIDSVID(object): #ilIDSVID数据集
    """
    iLIDS-VID
